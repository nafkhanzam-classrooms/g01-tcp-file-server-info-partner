    ...
    while True:
        try:
            ...
            if not msg:
                ...
                break
            handle_message(conn, msg, clients)
        except Exception as e:
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python server-thread.py <host> <port>")
        sys.exit(1)
    ...
    while True:
        try:
            ...
            clients.add(conn)
            t = threading.Thread(target=client_thread, args=(conn, addr, clients), daemon=True)
            t.start()
